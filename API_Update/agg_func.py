...
def refund_analysis(refund_data,order_data,inventory,cur_data):
    ...
    result_df = result_df.merge(order_data[['date','seller_id','order_amount','marketplace']], how = 'left',on = ['date','seller_id','marketplace'])
    result_df.columns = ['date','seller_id','market','退货产品总价','退货产品数量','退单数量','订单总数']

    result_df.to_csv('refund_data.csv',encoding = 'utf-8-sig')
...
def inbound_analysis(inbound_data,inventory,order_product):
    ...
    inbound.to_csv('inbound_data.csv',encoding = 'utf-8-sig')
